...
Cocktail tool comes with a very helpful `env` script that will save environment-variables set at build time to constants
...
- For a middle-of-the-road approach, you can divide configuration into two groups:
  ...
  * In this approach, you can use Angular's `fileReplacements` for anything environment-specific and Cocktail tool's
...
- If you don't have lots of environment variables, you can avoid dotenv-cli and use your particular shell's method
  to expose the variables before running the Cocktail tool env tool.
...
highlighting some Cocktail tool tooling that help with them in mind.
...
### Cocktail tool to the rescue

The Cocktail tool `env` task solves this problem really well, and avoids the need for separate `environment.ts` files for
...
## When you can use environment variables directly without Cocktail tool `env`

As a sidenote, Cocktail tool `env` isn't used for the proxy config file, because it isn't built and ran separately.
